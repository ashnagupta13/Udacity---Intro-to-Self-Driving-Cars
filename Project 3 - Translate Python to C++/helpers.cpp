...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	vector< vector<float> > newGrid(height, vector<float>(width, 0.0));
  	...
  	// Loop to find the sum of all elements in the grid
  	for(int i = 0; i < height; i++)
    {
  	...
  	// Loop to divide each element in th grid with the total value and store the new value in newGrid. 
  	for(int i = 0; i < height; i++)
    {
      for(int j = 0; j < width; j++)
      {
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
    ...
    // Looping through the grid and applying the window
  	for(int i = 0; i < height; i++)
    {
      for(int j = 0; j < width; j++)
      {
